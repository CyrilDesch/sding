...
./scripts/dev-docker.sh       # once: start Postgres + Langfuse
./scripts/dev-server.sh       # backend hot-reload
./scripts/dev-client.sh       # frontend Vite HMR (http://localhost:5173)
```
...
```bash
./scripts/dev-client.sh   # http://localhost:5173
```
...
All settings are read from environment variables. Copy `.env.example` to `.env`, then fill in `POSTGRES_*`. The dev scripts load `.env` automatically.
