...
class ProjectContextStateSpec extends AnyWordSpec with Matchers:
  ...
  "ProjectContextState" should {
    ...
    "increment iteration count correctly" in {
      val state          = ProjectContextState()
      val (state1, cnt1) = state.incrementIteration("test_node")
      cnt1 shouldBe 1
      state1.iterationCount("test_node") shouldBe 1

      val (state2, cnt2) = state1.incrementIteration("test_node")
      cnt2 shouldBe 2
      state2.iterationCount("test_node") shouldBe 2
    }

    "track multiple node iterations independently" in {
      val state           = ProjectContextState()
      val (state1, _)     = state.incrementIteration("node_a")
      val (state2, _)     = state1.incrementIteration("node_b")
      val (state3, cnt_a) = state2.incrementIteration("node_a")
      cnt_a shouldBe 2
      state3.iterationCount("node_a") shouldBe 2
      state3.iterationCount("node_b") shouldBe 1
    }
