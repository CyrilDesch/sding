    ...
    new LlmClient[F]:
      ...
      private def formatAiResponse(response: ChatResponse): String =
        ...
        if aiMsg.hasToolExecutionRequests() then
          ...
          Json.obj("role" := "assistant", "toolCall" := Json.obj("name" := req.name(), "arguments" := req.arguments())).noSpaces
        else
          Json.obj("role" := "assistant", "content" := Option(aiMsg.text()).getOrElse("")).noSpaces
      ...
      def chatStructured(
        ...
        withBaseAttrs(Tracer[F].spanBuilder("llm.chatStructured"), promptLink).build.use { span =>
          for
            ...
            result   = extractToolArguments(response)
      ...
      def chatStep(
        ...
        val toolSpecs    = tools.map(t => ToolSpecification.builder().name(t.name).description(t.description).build()).asJava
      ...
      def extractStructured(
        ...
        val request     = ChatRequest.builder()
          ...
          .toolChoice(ToolChoice.REQUIRED)
          .build()
        val inputJson = historyToInputJson(systemPrompt, fullHistory)
      ...
      private def extractToolArguments(response: ChatResponse): String =
        val aiMsg = response.aiMessage()
